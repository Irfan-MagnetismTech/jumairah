        ...
        <div style="width: 50%; float:left;">
            <div style="margin-top: 20px;">
                ...
                <p style="font-size: 12px; text-align: center; font-weight: bold; ">
                    Report For {{ $year ? date('Y', strtotime($year)) : date('F Y', strtotime($month))  }}
                </p>
    ...
    <div class="container" >
        @if(count($reportData))
            <table id="allowances">
                ...
                <tbody>
                    @foreach ($reportData as $key => $data)
                        <tr>
                            ...
                            <td>{{ $data->allowance_type->name }}</td>
                            <td>{{ date('Y-m-d', strtotime($data->created_at))}}</td>
                            <td class="text-right">@money($data->amount)</td>
                            {{-- <td class="text-center">{{ date('d-M-Y', strtotime($data->join_date)) }}</td> --}}
                ...
                <tfoot>
                    <tr>
                        <td colspan="5" class="text-right" style="width: 90%; font-weight:bold;">Total</td>

                        <td class="text-right">@money( $reportData->sum('amount') )</td>

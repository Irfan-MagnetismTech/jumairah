...
<a class="btn btn-out-dashed btn-sm btn-warning" href="{{ route('released-types.index') }}"><i
...
@section('content')
@if ($formType == 'create')
{!! Form::open(['url' => 'hr/released-types', 'method' => 'POST', 'class' => 'custom-form', 'files' =>
true,'enctype'=>'multipart/form-data']) !!}
@else
{!! Form::open([
'url' => "hr/released-types/$released_type->id",
'method' => 'PUT',
'class' => 'custom-form',
'files' => true,
...
<div class="row">
    <div class="col-12">
        <div class="input-group input-group-sm input-group-primary">
            <label style="min-width: 22% !important; max-width:22% !important;" class="input-group-addon"
                for="name">Released Type Name <span class="text-danger">*</span></label>
            {{ Form::text(
            'name',
            old('name')
            ? old('name')
            : (!empty($released_type->name)
            ? $released_type->name
            : null),
    ...
    <div class="col-12">
        <div class="input-group input-group-sm input-group-primary">
            ...
            old('status')
            ? old('status')
            : (!empty($released_type->status)
            ? $released_type->status
            : 'Active'),

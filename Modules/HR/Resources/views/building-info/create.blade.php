...
<a class="btn btn-out-dashed btn-sm btn-warning" href="{{ route('building-infos.index') }}"><i
...
@section('content')
@if ($formType == 'create')
{!! Form::open(['url' => 'hr/building-infos', 'method' => 'POST', 'class' => 'custom-form', 'files' =>
true,'enctype'=>'multipart/form-data']) !!}
@else
{!! Form::open([
'url' => "hr/building-infos/$buildingInfo->id",
'method' => 'PUT',
'class' => 'custom-form',
'files' => true,
...
<div class="row">
    <div class="col-12">
        <div class="input-group input-group-sm input-group-primary">
            ...
            old('name')
            ? old('name')
            : (!empty($buildingInfo->name)
            ? $buildingInfo->name
            : null),
            ['class' => 'form-control', 'id' => 'name', 'placeholder' => 'Enter Building name Here', 'required'],
            ) }}
            @error('name')
    ...
    <div class="col-12">
        <div class="input-group input-group-sm input-group-primary">
            ...
            old('short_name')
            ? old('short_name')
            : (!empty($buildingInfo->short_name)
            ? $buildingInfo->short_name
            : null),
    ...
    <div class="col-12">
        <div class="input-group input-group-sm input-group-primary">
            ...
            old('bangla')
            ? old('bangla')
            : (!empty($buildingInfo->bangla)
            ? $buildingInfo->bangla
            : null),
            ['class' => 'form-control', 'id' => 'bangla', 'placeholder' => 'Enter Bangla Name Here', 'required'],
            ) }}
            @error('bangla')

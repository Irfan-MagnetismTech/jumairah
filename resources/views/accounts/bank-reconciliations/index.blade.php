...
@section('breadcrumb-button')
    <a href="{{ route('bank-reconciliations-pdf') }}" class="btn btn-out-dashed btn-sm btn-danger"><i class="fa fa-file-pdf"></i></a>
        ...
        <table id="dataTable" class="table table-striped table-bordered">
            ...
            @foreach($transactions as $key => $data)
                    <tr>
                        ...
                        {!! Form::open(array('url' => "accounts/bank-reconciliations", 'method' => 'post')) !!}
                        <td>
                            @if(!empty($data->bankReconciliation))
                                ...
                                @endif
                            @else
                                <input type="text" name="date" class="trans_date" value="{{date('d-m-Y', strtotime(now()))}}" style="width: 70px">
                            @endif
                        ...
                        <td>
                            <div class="icon-btn">
                                @if(!empty($data->bankReconciliation))
                                    @if($data->bankReconciliation->status == 'Complete')
                                        <span class="badge bg-success badge-sm"> {{$data->bankReconciliation->status}} </span>
                                    @endif
                                @else
                        ...
                        {{--                        @if($loop->first)--}}
                            <td rowspan="">
                                <div class="icon-btn">
                                    <nobr>
                                        @if(empty($data->bankReconciliation))
                                            <button type="submit" data-toggle="tooltip" title="Posting"  class="btn btn-outline-warning"><i class="fas fa-check"></i></button>
                                        @endif
                                        {!! Form::close() !!}
{{--                                        <button type="button" class="btn btn-default modalShow" data-toggle="modal" data-target="" onclick="modalShow()">Static</button>--}}
                                        <a href="{{ route('vouchers.show',$data->id)}}" data-toggle="tooltip" title="View" class="btn btn-outline-primary"><i class="fas fa-eye"></i></a>
                                        <a href="{{ route("vouchers.edit", $data->id)}}" data-toggle="tooltip" title="Edit" class="btn btn-outline-warning"><i class="fas fa-pen"></i></a>
                                        {!! Form::open(array('url' => "accounts/vouchers/$data->id",'method' => 'delete', 'class'=>'d-inline','data-toggle'=>'tooltip','title'=>'Delete')) !!}

...
@section('breadcrumb-button')
    @can('project-create')
        <a href="{{ route('eme.show_load_cal', ['project' => $project]) }}" data-toggle="tooltip" title="Details" class="btn btn-out-dashed btn-sm btn-primary"><i class="fas fa-eye"></i></a>
        <a href="{{ route('eme.load_calculations.create') }}" class="btn btn-out-dashed btn-sm btn-success" data-toggle="tooltip" title="Create New"><i class="fa fa-plus"></i></a>
    @endcan
@endsection
                    ...
                    <span class='float-right pr-5 mr-5 d-flex'>
                         ...
                         <div class="icon-btn">
                            <nobr>
                                ...
                                <a href="{{ url("eme/load_calculations/approved/$data->id/1") }}" data-toggle="tooltip" title="Approve" class="btn btn-success"><i class="fa fa-check" aria-hidden="true"></i></a>
                                   
                                @endif
                                @if($data->approval()->doesntExist() || auth()->user()->hasAnyRole(['admin','super-admin']))
                                    
                                    <a href="{{ route('eme.load_calculations.edit', ['load_calculation' => $data]) }}" data-toggle="tooltip" title="Edit" class="btn btn-outline-warning">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    @if($data->approval()->doesntExist())
                                    <form action="{{ route('eme.load_calculations.destroy', ['load_calculation' => $data]) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')

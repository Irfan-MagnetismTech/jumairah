...
@section('content')
            <div class="row">
                <div class="col-md-12">
                    @if($formType == 'edit')
                        {!! Form::open(array('url' => "eme/load_calculations/$load_calculation->id",'method' => 'PUT', 'class'=>'custom-form')) !!}
                        <input type="hidden" name="id" value="{{old('id') ? old('id') : (isset($load_calculation) ? $load_calculation->id : null)}}">
                    @else
                        {!! Form::open(array('url' => "eme/load_calculations",'method' => 'POST', 'class'=>'custom-form')) !!}
                    @endif
                     <div class="col-md-6 col-xl-6 offset-md-3 pr-md-1 mb-2 pt-2">
                                <div class="input-group input-group-sm input-group-primary">
                                    <label class="input-group-addon" for="project_name">Project Name</label>
                                    {{Form::text('project_name',old('project_name') ? old('project_name') : (!empty($load_calculation) ? $load_calculation->project->name  : null),['class' => 'form-control','id' => 'project_name', 'placeholder'=>"Search Project Name", 'autocomplete'=>"off"])}}
                                    {{Form::hidden('project_id', old('project_id') ? old('project_id') : (!empty($load_calculation) ? $load_calculation->project_id  : null),['class' => 'form-control','id' => 'project_id','autocomplete'=>"off"])}}
                                </div>
                            </div>
                        <div class="row">
...
function appendCalculationRow(material_id,material_name) {
        ...
        $(document).on('keyup', ".material_name", function(){
                $(this).autocomplete({
                     source: function( request, response ) {
                         $.ajax({
                             url:"{{route('eme.load_calculations.materialAutoSuggestWhereDepthMorethanThree')}}",
                             type: 'post',
                             datatype: "json",
                             data: {
                                 _token: CSRF_TOKEN,
            ...
            $(document).on('keyup', ".floor_name", function(){
                ...
                let url = "{{url('boq/project')}}"+ "/" + project_id + "/departments/electrical/load_calculations/floorAutoSuggest"
                $(this).autocomplete({
                    source: function( request, response ) {
                        $.ajax({
                            url: url,

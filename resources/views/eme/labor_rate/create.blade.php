...
@section('breadcrumb-button')
    @include('components.buttons.breadcrumb-button', ['route' => route('eme.labor_rate.index'), 'type' => 'index'])
@endsection
...
<div class="row">
    <div class="col-md-12">
        @if ($formType == 'edit')
        <form action="{{ route('eme.labor_rate.update',['labor_rate' => $labor_rate]) }}" method="POST" class="custom-form">
        @method('put')
        @else
        <form action="{{ route('eme.labor_rate.store',) }}" method="POST" class="custom-form">
        @endif
        @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12 col-xl-12">
                                <div class="input-group input-group-sm input-group-primary">
                                    ...
                                    {{Form::text('project_name',old('project_name') ? old('project_name') : (!empty($labor_rate) ? $labor_rate->project->name  : null),['class' => 'form-control','id' => 'project_name', 'placeholder'=>"Search Project Name", 'autocomplete'=>"off"])}}
                                    {{Form::hidden('project_id', old('project_id') ? old('project_id') : (!empty($labor_rate) ? $labor_rate->project_id  : null),['class' => 'form-control','id' => 'project_id','autocomplete'=>"off"])}}
                                </div>
                            </div>
    ...
    const CSRF_TOKEN = "{{ csrf_token() }}";
        ...
        // function for searching third layer material
                $(document).on('keyup','.material_name',function(events){
                    ...
                    let url = "{{url('boq/project')}}"+ "/" + project_id + "/departments/electrical/labor_rate/materialSuggestWithLaborRate"
                    $(this).autocomplete({
        ...
        $(function() {
                ...
                $(document).on('keyup','.work_name',function(events){
                    ...
                    let project_id = $("#project_id").val();
                    let url = "{{url('boq/project')}}"+ "/" + project_id + "/departments/electrical/labor_rate/workSuggestWithLaborRate"
                    $(this).autocomplete({
                        source: function(request, response) {
                            $.ajax({
                                url: url,
        ...
        async function getSubWorkByWork(workId, trId) {
            try {
                ...
                let project_id = $("#project_id").val();
                let url = "{{url('boq/project')}}" +"/" + project_id + "/departments/electrical/configurations/rates/get_boq_sub_work_by_work_id"

...
@section('title', 'Admin Yearly Budget')

@section('breadcrumb-title')
    @if (!empty($adminYearlyBudget))
        Edit Admin Yearly Budget
    @else
        Create Admin Yearly Budget
    @endif
@endsection

@section('breadcrumb-button')
    <a href="{{ route('admin-yearly-budgets.index') }}" class="btn btn-out-dashed btn-sm btn-warning"><i
...
@section('content')
    ...
    @if (!empty($adminYearlyBudget))
        {!! Form::open(array('url' => "admin-yearly-budgets/$adminYearlyBudget->id",'encType' =>"multipart/form-data", 'method' => 'PUT', 'class'=>'custom-form')) !!}
    @else
        {!! Form::open(['url' => 'admin-yearly-budgets', 'method' => 'POST', 'class' => 'custom-form']) !!}
    @endif

{{--    <input type="text" name="id" value="{{!empty($adminYearlyBudget) ? $adminYearlyBudget->id : null}}">--}}
    <div class="row">
        <div class="col-md-6">
            <div class="input-group input-group-sm input-group-primary">
                <label class="input-group-addon" for="project_name">Date<span class="text-danger">*</span></label>
                {{ Form::text('date', old('date') ? old('date') : (!empty($adminYearlyBudget) ? $adminYearlyBudget->first()->date : now()->format('d-m-Y')), ['class' => 'form-control', 'id' => 'date', 'autocomplete' => 'off', 'required', 'placeholder' => 'Date' ]) }}
            </div>
        </div>
        <div class="col-md-6">
            <div class="input-group input-group-sm input-group-primary">
                <label class="input-group-addon" for="year">Budget For<span class="text-danger">*</span></label>
                {{ Form::text('year', old('year') ? old('year') : (!empty($adminYearlyBudget) ? $adminYearlyBudget->first()->year : now()->format('Y')), ['class' => 'form-control', 'id' => 'year', 'autocomplete' => 'off', 'required', 'placeholder' => 'Date', ]) }}
            </div>
        </div>
    </div><!-- end row -->
    ...
    <div class="table-responsive">
        <table class="table table-striped table-bordered table-sm text-center" id="itemTable">
            ...
            <tbody>
                ...
                @if (old('month'))
                    @foreach (old('month') as $key => $materialOldData)
                        <tr>
                            ...
                            <td>
                                <input type="number" name="remarks[]" value="{{ old('remarks')[$key] }}" min='0'
                ...
                @else
                    @if (!empty($adminYearlyBudget))
                        @foreach ($adminYearlyBudget->adminYearlyBudgetDetails  as $details)
                            <tr>
                                <td>
...
@section('script')
    <script>
        ...
        $(function() {
            @if (empty($adminYearlyBudget) && !old('project_id'))
                addRow();
            @endif

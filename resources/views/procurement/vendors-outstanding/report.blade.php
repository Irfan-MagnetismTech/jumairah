...
@section('breadcrumb-button')
    <a href="{{ route('vendors_outstanding_statement') }}" title="CLEAR" class="btn btn-out-dashed btn-sm btn-warning"><i class="fas fa-paint-roller"></i></a>
    <a href="{{ route('vendors-outstanding-statement-pdf',['dates'=>$dates])}}" title="PDF" class="btn btn-out-dashed btn-sm btn-success"><i class="fas fa-file-pdf"></i></a>
@endsection
    ...
    <div class="row">
        ...
        <div class="col-4 text-right">
            @if (isset($fromdate) && isset($todate))
                <h6>Date From {{ date('d-m-Y', strtotime($fromdate)) }} to {{ date('d-m-Y', strtotime($todate)) }}</h6>
            @endif
        </div>
    </div>
    <hr class="bg-success">

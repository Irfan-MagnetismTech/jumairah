...
@section('breadcrumb-button')
    <a href="{{ url('general-requisitions/create') }}" class="btn btn-out-dashed btn-sm btn-success"><i class="fa fa-plus"></i></a>
@endsection
    ...
    <div class="table-responsive">
        <table id="dataTable" class="table table-striped table-bordered">
            ...
            @foreach($requisitions as $key => $requisition)
             @if ( auth()->user()->project_assigned == 0 || (auth()->user()->project_assigned == 1 && (in_array($requisition->cost_center_id, auth()->user()->assignedProject->pluck('cost_center_id')->toArray()))))
                <tr>
                    <td>{{ $indx++ }}</td>
                    <td> MPR - {{$requisition->mpr_no}}</td>
                    <td>
                        ...
                        <div class="icon-btn">
                            <nobr>
                                ...
                                @if((!empty($approval) && $approval->designation_id == auth()->user()->designation?->id && $approval->department_id == auth()->user()->department_id) || (!empty($approval) && auth()->user()->hasAnyRole(['admin','super-admin'])))
                                    <a href="{{ url("general-requisitions/approved/$requisition->id/1") }}" data-toggle="tooltip" title="Approve Requisition" class="btn btn-success"><i class="fa fa-check" aria-hidden="true"></i></a>
                                @endif
                                @if($requisition->approval()->doesntExist() || auth()->user()->hasAnyRole(['admin','super-admin']))
                                    <a href="{{ url("general-requisitions/$requisition->id/edit") }}" data-toggle="tooltip" title="Edit" class="btn btn-outline-warning"><i class="fas fa-pen"></i></a>
                                    ...
                                    @endif
                                @endif
                                <a href="{{ url('general-requisition-pdf') }}/{{ $requisition->id }}" title="PDF" class="btn btn-out-dashed btn-sm btn-success"><i class="fas fa-file-pdf"></i></a>
                                <a href="{{ url("general-requisitions/$requisition->id") }}" data-toggle="tooltip" title="Details" class="btn btn-outline-primary"><i class="fas fa-eye"></i></a>
                                <a href="{{ url("general-requisition-Log/$requisition->id/log") }}" data-toggle="tooltip" title="Logs" class="btn btn-dark"><i class="fas fa-history"></i></a>

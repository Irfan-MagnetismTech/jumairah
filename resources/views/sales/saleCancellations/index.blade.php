...
@section('breadcrumb-button')
    ...
    <a href="{{ url('saleCancellations/create') }}" class="btn btn-out-dashed btn-sm btn-success"><i class="fa fa-plus"></i></a>
    @endcan
@endsection
                ...
                @foreach($saleCancellations as $saleCancellation)
                    <tr>
                        ...
                        <td class="breakWords text-left">
                            <a href="{{route('clients.show', $saleCancellation->sell->sellClient->Client->id)}}" data-toggle="tooltip" title="Check Client Profile">
                        ...
                        <td>
                        <h6 class="m-0">
                                <a href="{{route('sells.show', $saleCancellation->sell->id)}}" data-toggle="tooltip" title="Check Sale Information">
                        ...
                        <td>
                            @if($saleCancellation->approval()->exists())
                                ...
                                @foreach($saleCancellation->approval as $approval)
                                    ...
                                    {{$approvalstatus = $approval->status }} - {{$approval->user->employee->designation->name}}
                                </span><br>
                                @endforeach
                            @else
...
{{--                                    {{$approvalstatus}}--}}
                                    @if(!empty($approval) && $approval->designation_id == auth()->user()->designation?->id )
                                        @if($approvalstatus == 'Pending' || $approvalstatus == 'Approved')
                                            <a href="{{ url("saleCancellations/{$saleCancellation->id}/approve") }}" data-toggle="tooltip" title="Approve Request" class="btn btn-primary"><i class="fas fa-check"></i></a>
                                        @endif
                                    @endif

                                    @can('sell-view')
                                    <a href="{{ url("saleCancellations/$saleCancellation->id") }}" data-toggle="tooltip" title="Show" class="btn btn-outline-primary"><i class="fas fa-eye"></i></a>
                                    @endcan
{{--                                    {{$approvalstatus}}<br>--}}
                                    @if($saleCancellation->approval()->doesntExist())
                                        @can('sell-edit')
                                            <a href="{{ url("saleCancellations/$saleCancellation->id/edit") }}" data-toggle="tooltip" title="Edit" class="btn btn-outline-warning"><i class="fas fa-pen"></i></a>
                                        @endcan
                                        @can('sell-delete')
                                            {!! Form::open(array('url' => "saleCancellations/$saleCancellation->id",'method' => 'delete', 'class'=>'d-inline','data-toggle'=>'tooltip','title'=>'Delete')) !!}
                                    ...
                                    @endif
                                    {!! Form::close() !!}
                                    <a href="{{ url("saleCancellations/$saleCancellation->id/log/") }}" data-toggle="tooltip" title="Log" class="btn btn-dark"><i class="fas fa-history"></i></a>
                                </nobr>
                            </div>
                        </td>
                    </tr>

...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <form
                action="{{ route('boq.project.departments.civil.cost_analysis.floor_wise.material.summary', ['project' => $project]) }}"
                method="get">
                <div class="row px-2">
                    <div class="col-md-4 px-1 my-1 my-md-0" data-toggle="tooltip" title=""
                        ...
                        <select class="form-control form-control-sm select2" autocomplete="off" name="nested_material_id">
                            ...
                            @foreach ($material_list as $material)
                                @if($material?->nestedMaterial?->material_status != "Work-Material")
                                <option value="{{ $material->material_id }}"
                                    {{ request()->input('nested_material_id') == $material->material_id ? 'selected' : '' }}>
                                    {{ $material?->nestedMaterial?->name }}
                                </option>
                                @endif
                            @endforeach
                        </select>
                ...
                <a style="float: right" target="_blank"
                    href="{{ route('boq.project.departments.civil.cost_analysis.floor_wise.download.material.summary', ['project' => $project]) }}"
                ...
                <table id="work-table" class="table table-striped table-bordered">
                    ...
                    <tbody>
                        ...
                        @foreach ($material_statements as $material_statement_key => $material_statement)
                            @if($material_statement['material_status'] != "Work-Material")
                            @foreach ($material_statement['floors'] as $material_key => $material)
                                ...
                                <tr style="background-color: {{ $row_bg_color }};">
                                    ...
                                    <td style="font-weight: bold" width="30%">
                                        {{ \Illuminate\Support\Str::limit($material?->boqCivilCalcProjectFloor?->boqCommonFloor?->name, 50) ?? $material?->floor_id }}
                                    </td>

                        ...
                        <div class="row">
                            <div class="col-xl-12 col-md-12" id="works-table">
                                <div class="input-group input-group-sm input-group-primary" id="work-0">
                                    ...
                                    <select class="form-control work_id first_work" id="work_id" name="work_id[]" required>
...
@section('script')
    ...
    <script>
        ...
        const REMOVE_MATERIAL_SUPREME_REVISED = "{{ route('boq.project.departments.civil.remove.budget.from.supreme.revised', $project) }}";
...
`;
        ...
        async function getPreviousCalculations(workDetails, copyFromFloorId = null) {
            try {
                ...
                const formData = {
                    _token: "{{ csrf_token() }}",
                    work_id: workDetails?.id ?? -1,
                    boq_floor_id: copyFromFloorId === null ? boqFloorId : copyFromFloorId,
                    calculation_type: CALCULATION_TYPE,
                };
        ...
        $(document).on('submit', '#form', function(event) {
            ...
            formData["boq_floor_id"] = $('#boq_floor_id').val();
            formData["is_reinforcement"] = is_reinforcement;
            formData["boq_floor_type_id"] = $('#location-type').val();
            formData["work_type_id"] = parseInt($('.first_work').val());
            formData["work_id"] = parseInt($('.work_id').last().val());
        ...
        $(document).on('select2:unselect', '.group_wise_selected_material', function (e) {

            let materialId = e.params.data.id;
            let boqFloorId = $('#boq_floor_id').val();
            ...
            removeFloorMaterialData(formData);

        });

        //delete material data from supreme and revised sheet
        async function removeFloorMaterialData(formData){
            const response = await axios.post(REMOVE_MATERIAL_SUPREME_REVISED, formData);
        }

                ...
                <li class="pcoded-hasmenu {{ request()->route()->parameter('boq_area')?->boqCommonFloor->type_name == $type_name
                    ...
                    <ul class="pcoded-submenu">
                        @foreach ($sidebar_boq_area as $sidebar_area)
                            <li class="{{ request()->route()->parameter('boq_area')?->area_uid == $sidebar_area->area_uid
                                ? 'active pcoded-trigger'
                                : null }}">
                                <a href="{{ route('boq.project.departments.civil.materialsAndLabours.calculations.create', ['project' => $project, 'boq_area' => $sidebar_area]) }}">

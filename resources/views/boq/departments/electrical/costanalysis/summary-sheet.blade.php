...
@section('breadcrumb-title')
    Material Summary List [Electrical]
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <form
                action="{{ route('boq.project.departments.electrical.cost_analysis.floor_wise.material.summary', ['project' => $project]) }}"
                method="get">
                <div class="row px-2">
                        ...
                        <select class="form-control form-control-sm select2" autocomplete="off" name="nested_material_id">
                            ...
                            @foreach ($material_list as $material)
                                <option value="{{ $material->material_id }}"
                                    ...
                                    {{ $material?->nestedMaterial?->name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <a style="float: right" target="_blank"
                    href="{{ route('boq.project.departments.electrical.cost_analysis.floor_wise.download.material.summary', ['project' => $project]) }}"
                            ...
                            @foreach ($material_statement['floors'] as $material_key => $material)
                                ...
                                <tr style="background-color: {{ $row_bg_color }};">
                                    ...
                                    <td style="font-weight: bold" width="30%">
                                        {{ \Illuminate\Support\Str::limit($material?->BoqFloorProject?->boqCommonFloor?->name, 50) }}
                                    </td>

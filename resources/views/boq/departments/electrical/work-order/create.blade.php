...
@section('breadcrumb-button')
    @if ($formType == 'edit')
        <a href="{{route("boq.project.departments.electrical.workorder.terms", ['project' => $project ,'workorder' => $work_order->id])}}" class="btn btn-out-dashed btn-sm btn-info" data-toggle="tooltip" title="Edit Terms"><i class="fas fa-long-arrow-alt-right"></i></a>
    @endif
...
@section('content')
    {!! Form::open(['url' => empty($work_order) ? route('boq.project.departments.electrical.work_order.store',['project' => $project]) : route('boq.project.departments.electrical.work_order.update',['project' => $project,'work_order' => $work_order->id]), 'method' => empty($work_order) ? 'POST' : 'PUT', 'class' => 'custom-form']) !!}
...
@section('script')
    ...
    <script>
        ...
        function getCsSupplier() {
            ...
            if (cs_supplier != '') {
                const url = "{{ url('boq/project/$project->id/departments/electrical/workorder/loadWorkCsSupplier') }}/" + cs_supplier;
        ...
        $("#cs_no").autocomplete({
            source: function(request, response) {
                $.ajax({
                    url: "{{route('boq.project.departments.electrical.workorder.workCSRefAutoSuggest',$project->id)}}",
                    type: 'post',
                    dataType: "json",
                    data: {
                        _token: CSRF_TOKEN,
